...
public class RasterReader {
     ...
     * @throws IOException
     */
    public int[] readAsIntArray(int nanValue) throws IOException {
        double[] dvalues = readAsDoubleArray();
        return IntStream.range(0, dvalues.length).map(i -> {
            double v = dvalues[i];
            if (Double.isNaN(v)) {
                return nanValue;
            }
